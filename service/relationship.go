...
func (s *Service) ListRelationships(ctx context.Context, user *model.User, status string) ([]response.RelationshipResponse, error) {
	...
	if status != "" {
		parsed, err := parseRelationshipStatus(status)
		if err != nil {
			return nil, err
		}
	...
	out := make([]response.RelationshipResponse, 0, len(relationships))
...
func (s *Service) RequestRelationship(ctx context.Context, actorKey string, partnerKey string) (*response.RelationshipResponse, error) {
...
func (s *Service) AcceptRelationship(ctx context.Context, userKey string, actorKey string) (*response.RelationshipResponse, error) {
...
func (s *Service) relationshipUserPair(ctx context.Context, userKey string, targetUserKey string) (*model.User, *model.User, error) {
	...
	if strings.TrimSpace(targetUserKey) == "" {
		return nil, nil, appError(http.StatusBadRequest, "target is required")
	}
...
func parseRelationshipStatus(raw string) (model.RelationshipStatus, error) {
	status := model.RelationshipStatus(strings.TrimSpace(raw))
	switch status {
	case "", model.RelationshipStatusActive, model.RelationshipStatusPending:
		return status, nil
	default:
		return "", appError(http.StatusBadRequest, "invalid relationship status")
	}
}
...
func relationshipResponse(relationship *model.Relationship) response.RelationshipResponse {
	return response.RelationshipResponse{
		Key:       relationship.Key.String(),
		Status:    string(relationship.Status),
		Actor:     httpresp.Refine[*model.User, response.ProfileResponse](&relationship.Actor),
		Partner:   httpresp.Refine[*model.User, response.ProfileResponse](&relationship.Partner),
